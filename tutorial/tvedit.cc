...
TDialog *TVDemo::createFindDialog()
{
	...
	d->insert(new
		  TButton(TRect(14 + 12, 9, 24 + 12, 11), "Cancel", cmCancel,
			  bfNormal));
	d->selectNext(False);
	return d;
}
...
ushort TVDemo::doEditorDialog(int dialog, ...)
{
	...
	case edOutOfMemory:
		return messageBox("Not enough memory for this operation",
				  mfError + mfOKButton);
	case edReadError:
		return messageBox(mfError + mfOKButton, "Error reading file %s",
				  info);
	case edWriteError:
		return messageBox(mfError + mfOKButton, "Error writing file %s",
				  info);
	case edCreateError:
		return messageBox(mfError + mfOKButton,
				  "Error creating file %s", info);
	case edSaveModify:
...
TMenuBar *TVDemo::initMenuBar(TRect r)
{
	...
	TSubMenu & sub2 = *new TSubMenu("~F~ile", 0, hcNoContext) +
	    ...
	    *new TMenuItem("~C~hange dir...", cmChDir, kbNoKey,
			   hcNoContext, "") +
	    *new TMenuItem("~S~suspend", cmSuspend, kbNoKey,
			   hcNoContext, "") +
	...
	TSubMenu & sub3 = *new TSubMenu("~E~dit", 0, hcNoContext) +
	    *new TMenuItem("~U~ndo", cmUndo, kbNoKey, hcNoContext, "") +
	    newLine() +
	    *new TMenuItem("Cu~t~", cmCut, kbShiftDel, hcNoContext,
			   "Shift-Del") +
	    *new TMenuItem("~C~opy", cmCopy, kbCtrlIns, hcNoContext,
			   "Ctrl-Ins") +
	    ...
	    *new TMenuItem("~S~how clipboard", cmShowClip, kbNoKey,
			   hcNoContext, "") +
	    newLine() +
	    *new TMenuItem("~C~lear", cmClear, kbCtrlDel, hcNoContext,
			   "Ctrl-Del");
