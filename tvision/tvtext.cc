...
inline uchar lo(ushort w)
{
	return w & 0xff;
}

inline uchar hi(ushort w)
{
	return w >> 8;
}
...
ushort getCtrlCode(uchar ch)
{
	return getAltCode(ch) |
	    (((('a' <= ch) && (ch <= 'z')) ? (ch & ~0x20) : ch) - 'A' + 1);
}

const char *TRadioButtons::button = " ( ) ";

const char *TMenuBox::frameChars =
...
char TFrame::frameChars[33] = "   À ³ÚÃ ÙÄÁ¿´ÂÅ   È ºÉÇ ¼ÍÏ»¶Ñ ";	// for UnitedStates code page
...
const char *TStringLookupValidator::errorMsg =
