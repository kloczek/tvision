...
ushort messageBoxRect(const TRect & r, ushort aOptions, const char *fmt, ...)
{
	va_list argptr;
	va_start(argptr, fmt);
...
ushort inputBoxRect(const TRect & bounds,
		    const char *Title, const char *aLabel, char *s, uchar limit)
{
	TDialog *dialog;
	TView *control;
	TRect r;
	...
	r.b.x += 12;
	dialog->
	    insert(new TButton(r, MsgBoxText::cancelText, cmCancel, bfNormal));
