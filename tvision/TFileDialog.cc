...
TFileDialog::TFileDialog(const char *aWildCard,
			 ...
			 ushort aOptions,
			 uchar histId):TWindowInit(&TFileDialog::initFrame),
TDialog(TRect(15, 1, 64, 20), aTitle), directory(newStr(""))
{
	options |= ofCentered;
			...
			if (isWild(fName)) {
				...
				if (checkDirectory(path)) {
					...
					if (command != cmFileInit)
						fileList->select();
					fileList->readDirectory(directory,
								wildCard);
				}
			} else if (isDir(fName)) {
