...
TFileInputLine::TFileInputLine(const TRect & bounds,
			       short aMaxLen):TInputLine(bounds, aMaxLen)
{
...
void TFileInputLine::handleEvent(TEvent & event)
{
	...
	if (event.what == evBroadcast &&
	    event.message.command == cmFileFocused && !(state & sfSelected)
	    ) {
#ifndef __UNPATCHED
		...
		if ((((TSearchRec *) event.message.infoPtr)->attr & FA_DIREC) !=
			...
			if (!strchr(data, ':') && !strchr(data, '/')) {
				strcpy(data,
				       ((TSearchRec *) event.message.infoPtr)->
				       name);
				strcat(data, "/");
		...
		if ((((TSearchRec *) event.message.infoPtr)->attr & FA_DIREC) !=
		    0) {
			strcpy(data,
			       ((TSearchRec *) event.message.infoPtr)->name);
			strcat(data, "/");
			strcat(data, ((TFileDialog *) owner)->wildCard);
		}
#endif
...
TSortedListBox::TSortedListBox(const TRect & bounds,
			       ushort aNumCols,
			       TScrollBar *aScrollBar):TListBox(bounds,
								aNumCols,
								aScrollBar),
shiftState(0), searchPos(-1)
{
	showCursor();
	setCursor(1, 0);
}
...
TFileInfoPane::TFileInfoPane(const TRect & bounds):TView(bounds)
{
	eventMask |= evBroadcast;
}
