...
class TMenuItem {
	...
	TMenuItem(const char *aName,
		  ...
		  const char *p = nullptr, TMenuItem * aNext = nullptr);
    /**
     ...
     */
	 TMenuItem(const char *aName,
		   ushort aKeyCode,
		   TMenu * aSubMenu,
		   ushort aHelpCtx = hcNoContext, TMenuItem * aNext = nullptr);
    /**
...
inline TMenuView::TMenuView(const TRect & bounds,
			    TMenu *aMenu,
			    TMenuView *aParent):TView(bounds),
parentMenu(aParent), menu(aMenu), current(nullptr)
{
	eventMask |= evBroadcast;
}

inline TMenuView::TMenuView(const TRect & bounds):TView(bounds),
parentMenu(nullptr), menu(nullptr), current(nullptr)
{
	eventMask |= evBroadcast;
}
...
class TStatusItem {
	...
	TStatusItem(const char *aText,
		    ushort key, ushort cmd, TStatusItem * aNext = nullptr);
    /**
     * Undocumented.
     */
	~TStatusItem();
...
inline TStatusItem::TStatusItem(const char *aText,
				ushort key,
				ushort cmd,
				TStatusItem *aNext):next(aNext),
text(newStr(aText)), keyCode(key), command(cmd)
{
}
...
inline TStatusDef::TStatusDef(ushort aMin,
			      ...
			      TStatusDef *aNext):next(aNext),
min(aMin), max(aMax), items(someItems)
{
}
