...
TInputLine::TInputLine(const TRect & bounds, int aMaxLen,
		       TValidator *aValid):TView(bounds),
data(new char[aMaxLen]), maxLen(aMaxLen - 1), curPos(0), firstPos(0),
selStart(0), selEnd(0), validator(aValid), anchor(-1),
#ifndef __UNPATCHED
oldAnchor(-1),
#endif
...
Boolean TInputLine::checkValid(Boolean noAutoFill)
{
	...
	if (validator) {
		...
		if (!validator->isValidInput(newData, noAutoFill)) {
			...
			strcpy(data, newData);
			if ((curPos >= oldLen)
			    && (static_cast < int >(strlen(data)) > oldLen))
				curPos = strlen(data);
			delete newData;
			return True;
		}
	...
	if ((state & sfSelected) != 0)
		...
		case evKeyDown:
			...
			case kbDel:
				...
				break;
			case kbIns:
				setState(sfCursorIns,
					 Boolean(!(state & sfCursorIns)));
				break;
			default:
...
{
	TView::write(os);
	os << maxLen << curPos << firstPos << selStart << selEnd;
	os.writeString(data);
	os << validator;
}
...
{
	TView::read(is);
	is >> maxLen >> curPos >> firstPos >> selStart >> selEnd;
