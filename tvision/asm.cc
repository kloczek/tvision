...
{
	if (lockRefresh != 0)
		return;		/* we can't do any refresh */
	if (p->owner != nullptr && p->owner->lockFlag)
		return;		/* the owner is locked */
	refresh();
}
...
size_t scan(const void *block, size_t size, const char *str)
{
	...
	if (patternLength <= 0)
		return UINT_MAX;	//SS: nothing to search
...
size_t iScan(const void *block, size_t size, const char *str)
{
	...
	while (1) {
		...
		testHash =
		    ((testHash << 5) + toupper(*(patternLength + testP++))) % q;
	}
	return UINT_MAX;	// Not found.
};
...
#if 0
void TEditor::formatLine(void *DrawBuf, uint LinePtr, int Width, ushort Color)
{
	uint p = LinePtr;
	while ((p < curPtr) && (buffer[p] != 0x0D) && (p - LinePtr <= Width)) {
		((ushort *) DrawBuf)[p - LinePtr] =
...
void TEditor::formatLine(void *DrawBuf, uint LinePtr, int Width, ushort Color)
{
	...
	if (p >= curPtr) {
		...
		while ((p < bufSize) && (buffer[p] != '\n') && (i <= Width)) {
			curColor = (p >= selStart
				    && p <
				    selEnd) ? (Color & 0xFF00) : ((Color & 0xFF)
								  << 8);
			if (buffer[p] == 0x9) {
				do {
					((ushort *) DrawBuf)[i] =
...
/*20*/
		...
		if (!(p->state & sfVisible) || staticVars2.y < p->origin.y)
			continue;	// keine Verdeckung

		if (staticVars2.y < p->origin.y + p->size.y) {
			// šberdeckung m”glich.
			if (x1 < p->origin.x) {	// f„ngt links vom Object an.
				if (x2 <= p->origin.x)
					continue;	// links vorbei
				if (x2 > p->origin.x + p->size.x) {
					if (exposedRec1(x1, p->origin.x, p))
						return 1;
...
void TFrame::frameLine(TDrawBuffer & frameBuf, short y, short n, uchar color)
{
	...
	while (1) {
		...
		if ((p->options & ofFramed) && (p->state & sfVisible)) {
			...
			if (y + 1 < p->origin.y)
				continue;
			else if (y + 1 == p->origin.y) {
				mask1 = 0x0A;
				mask2 = 0x06;
			} else if (y == p->origin.y + p->size.y) {
				mask1 = 0x0A;
				mask2 = 0x03;
			} else if (y < p->origin.y + p->size.y) {
				mask1 = 0;
	...
	while (i--) {
		*dest++ =
		    ...
		    (unsigned char)frameChars[frameMask[i1]];
		i1++;
	}			/* endwhile */
};
...
/* SS: draws mouse pointer */
			...
			if (p->owner->lockFlag == 0)
				writeViewRec2(x1, x2, p->owner, shadowCounter);
		...
		if (staticVars2.y < p->origin.y + p->size.y) {
			...
			if ((p->state & sfShadow)
				...
				if (x1 >=
				    p->origin.x + p->size.x + shadowSize.x) {
		...
		if ((p->state & sfShadow)
			...
			if (x1 < p->origin.x + shadowSize.x) {
				if (x2 <= p->origin.x + shadowSize.x)
					continue;	// links vorbei
				writeViewRec1(x1, p->origin.x + shadowSize.x, p,
					      shadowCounter);
...
{
	if (!(p->state & sfVisible) || p->owner == nullptr)
		return;
